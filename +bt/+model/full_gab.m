...
classdef full_gab < bt.model.template
	...
	methods
		function self = full_gab() % Constructor
		...
		function set_cache(self,initial_values) % Calculate the cache - takes in skip_fit
			...
			if self.skip_fit(self.t0_index) == 2 && ~isempty(initial_values) % If t0 is not being fitted, then the value is of course fixed
		...
		function prepare_for_fit(self,varargin)
			...
			if nargin < 6 || isempty(varargin{5})
				self.default_t0_skip_fit(varargin{1},varargin{2},self.t0_index);
			else
