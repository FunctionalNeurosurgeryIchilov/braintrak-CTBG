classdef full_gab_spatial2d < bt.model.template_spatial
	...
	methods
		function self = full_gab_spatial2d() % Constructor
			self.name = 'full_gab_spatial2d';
            ...
            self.param_symbols = {'G_{ee}','G_{ei}','G_{es}','G_{se}','G_{sr}','G_{sn}','G_{re}','G_{rs}','\alpha','\beta','t_0','A_{EMG}',...
                                 ...
                                 'Alpha_{x_var}','Alpha_{y_var}','Alpha_{x_ph}','Alpha_{y_ph}',...
                                 'Beta_{x_var}','Beta_{y_var}','Beta_{x_ph}','Beta_{y_ph}',...
            ...
            p = model.params();
            self.initial_step_size = [self.initial_step_size  0.1  0.1  0.05  0.05   0.1  0.1  0.05  0.05   0.2  0.2  0.05  0.05   0.5  0.5  0.05  0.05      3    3  0.05  0.05    0.001  0.001  0.05  0.05]; 
            self.limits =            [self.limits             [-4   -4     0     0    -4   -4     0     0   -10  -10     0     0   -40  -40     0     0   -300 -300     0     0    -0.03  -0.03     0     0;...
                                                                4    4  p.Lx  p.Ly     4    4  p.Lx  p.Ly    10   10  p.Lx  p.Ly    40   40  p.Lx  p.Ly    300  300  p.Lx  p.Ly     0.03   0.03  p.Lx  p.Ly]];             
		...
		function set_params(self,pars)
            ...
            self.p.apply_variation('alpha','costume_doc_2d_phased',pars(25),pars(26),pars(27),pars(28));
            self.p.apply_variation('beta','costume_doc_2d_phased',pars(29),pars(30),pars(31),pars(32));
            self.p.apply_variation('t0','costume_doc_2d_phased',pars(33),pars(34),pars(35),pars(36));
        end
		...
		function [initial_values,prior_pp] = initialize_fit(self,target_f,target_P) % Return the first posterior/prior and initial values
            initial_vals_variation = zeros(1,6*4);
